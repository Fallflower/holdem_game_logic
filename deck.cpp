...
std::string Deck::pubCardsStr(int stateCode) const {
    ...
    switch (stateCode) {
    case 3: temp[4] = pubCards_[4].to_string(); [[fallthrough]];
    case 2: temp[3] = pubCards_[3].to_string(); [[fallthrough]];
    case 1:
        temp[0] = pubCards_[0].to_string();
        temp[1] = pubCards_[1].to_string();
        temp[2] = pubCards_[2].to_string();
        break;
    default: break;
    }
